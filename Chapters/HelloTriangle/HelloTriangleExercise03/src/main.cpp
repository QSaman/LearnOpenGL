...
const char* fragmentShaderSrc[2] =
{
"#version 330 core\n"
"out vec4 FragColor;\n"
"void main()\n"
...
GLuint createFragmentShader(int fragmentShaderIndex)
{
	auto shader = glCreateShader(GL_FRAGMENT_SHADER);
	glShaderSource(shader, 1, &fragmentShaderSrc[fragmentShaderIndex], NULL);
	return shader;
}
...
bool setupShaders(GLuint shaderProgram, int fragmentShaderIndex)
{
	auto vertexShader = createVertexShader();
	if (!compileShader(vertexShader, "vertex shader"))
		return false;
	auto fragmentShader = createFragmentShader(fragmentShaderIndex);
...
int main()
{
	...
	for (int i = 0; i < 2; ++i)
	{
		...
		if (!setupShaders(shaderProgram[i], i))
			return 1;
	}
    ...
    while (!glfwWindowShouldClose(window))
    {
		...
		for (int i = 0; i < 2; ++i)
			renderFrame(shaderProgram[i], vao[i]);
