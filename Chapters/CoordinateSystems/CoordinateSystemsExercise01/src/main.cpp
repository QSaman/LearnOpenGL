...
void renderFrame(ShaderLoader& shader, GLuint vao, GLuint* texture, int texNum)
{
	...
	glm::mat4 projection = glm::perspective(fov, aspect_ratio, nearPlane, farPlane);
...
int main()
{
	std::cout << "Press up/down to increase/decrease FOV" << std::endl;
	std::cout << "Press right/left to increase/decrease aspect ratio" << std::endl;
	std::cout << "Press n/m to increase/decrease near plane" << std::endl;
	std::cout << "Press f/g to increase/decrease far plane" << std::endl << std::endl << std::endl;
	print_var(fov);
	print_var(aspect_ratio);
	print_var(nearPlane);
	print_var(farPlane);
